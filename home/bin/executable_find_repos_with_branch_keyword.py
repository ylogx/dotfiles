...
def scan_repo(
    path: Path, keyword: str, case_sensitive: bool, timeout_git: float
...
def main():
    ...
    ap.add_argument(
        ...
        "-k",
        default="telemetry",
        help="Keyword to search in branch names (default: telemetry)",
    )
    ap.add_argument(
        "--case-sensitive",
        action="store_true",
    ...
    ap.add_argument(
        ...
        type=float,
        default=5.0,
        help="Per git command timeout seconds (default: 5.0)",
    )
    ap.add_argument(
        "--follow-symlinks",
        action="store_true",
        help="Follow symlinks during directory walk",
    )
    ap.add_argument(
        "--json", action="store_true", help="Emit JSON with repo and matching branches"
    )
    args = ap.parse_args()
        ...
        payload = [
            {
                ...
                "matching": [
                    ...
                    if (
                        ...
                        else args.keyword.lower() in b.lower()
                    )
                ],
                "all_branches_count": len(branches),
            }
